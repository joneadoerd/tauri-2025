...
function createBasePacketFlags(): PacketFlags {
  return { isFragmented: false, isEncrypted: false, isCompressed: false, isSigned: false };
}
...
export const PacketFlags: MessageFns<PacketFlags> = {
  encode(message: PacketFlags, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.isFragmented !== false) {
      writer.uint32(8).bool(message.isFragmented);
    }
    if (message.isEncrypted !== false) {
      writer.uint32(16).bool(message.isEncrypted);
    }
  ...
  fromJSON(object: any): PacketFlags {
    return {
      isFragmented: isSet(object.isFragmented) ? globalThis.Boolean(object.isFragmented) : false,
      isEncrypted: isSet(object.isEncrypted) ? globalThis.Boolean(object.isEncrypted) : false,
  ...
  toJSON(message: PacketFlags): unknown {
    ...
    if (message.isEncrypted !== false) {
      obj.isEncrypted = message.isEncrypted;
    }
    if (message.isCompressed !== false) {
  ...
  fromPartial<I extends Exact<DeepPartial<PacketFlags>, I>>(object: I): PacketFlags {
    ...
    message.isEncrypted = object.isEncrypted ?? false;
